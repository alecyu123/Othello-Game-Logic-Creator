...
def game_over(board):
    ...
    Checks if the player and the computer have any moves left
    Returns True if neither do
    '''
    if len(_get_legal_moves(board, HUMAN)) == 0 and len(_get_legal_moves(board, COMPUTER)) == 0: # Checks if either party still has any legal moves left
        return True
...
def game_winner(board):
    ...
    Compares the number of human pieces on the board to those of computer pieces on the board.
    In the case of a tie, returns 0
    '''
    if board_count(board, HUMAN) > board_count(board, COMPUTER):   # Checks if there are more HUMAN pieces than COMPUTER pieces
        return HUMAN
    elif board_count(board, HUMAN) < board_count(board, COMPUTER): # Checks if there are more COMPUTER pieces than HUMAN pieces
        return COMPUTER
    else:
        return 0
#
...
def _choose_move(board, moves, player):
    '''
    board (list): an object returned by board_create()
    moves (list): list created by _get_legal_moves function containing nested tuples that describe possible moves with row and column
    player (int): HUMAN or COMPUTER
    '''
    lst = []  # Creates an empty list to store potentially good moves
    count = 0
    for move in moves: # Checks for all potential move tuples in list moves
        if len(_get_flips(board, move[0], move[1], player, _get_opponent(player))) > count: # If the number of flips is higher than the initial count, which it will be for at least the first legal move, it will append it to lst
            lst.append(move)
            count = len(_get_flips(board, move[0], move[1], player, _get_opponent(player))) # Increase the number of flips required to find the best possible greedy move
    return lst[-1] # Returns the last element of the list, which is the one with the most flips for the next move
