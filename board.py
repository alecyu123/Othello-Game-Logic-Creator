...
def board_count(board, value):
    '''
    board (list): an object created by the function board_create()
    value(int): value that will be checked in the nested lists

    Return the number of squares on the board that are equal to the given value.
    '''
    count = 0
    for row in board:         # Nested for loop goes through the lists in the list to check for same value.
        for code in row:
            if code == value:
                count += 1    # Adds to the count of the value, which started at 0.
    return count
